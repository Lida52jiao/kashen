...
public class MonitorTask {
	...
	public void task() throws Exception {
		...
		String centent = "当前时间是：" + CommonUtil.fromDateH() + "<br/><br/>"
				...
				+ "padding-left:7px;" + "padding-right:7px;" + "width:15px;"
				+ "text-align:center;" + "}" +
				...
				+ "<td width=\"100\">名称</td>" + "<td width=\"100\">参数</td>"
				+ "<td width=\"275\">预警设置</td>" + "</tr>" + "</thead>"
				+ "<tbody id=\"tbody\">" + "<tr " + cpubool
				+ "><td>CPU</td><td style=\"text-align: left;\">当前使用率："
				+ cpuUsage + "%</td><td>使用率超出  " + cpu + "%,,发送邮箱提示 </td></tr>"
				+ "<tr " + rambool
				+ "><td>服务器内存</td><td style=\"text-align: left;\">当前使用率："
				+ serverUsage + "%</td><td>使用率超出  " + ram
				+ "%,发送邮箱提示 </td></tr>" + "<tr " + jvmbool
				+ "><td>JVM内存</td><td style=\"text-align: left;\">当前使用率："
				+ jvmUsage + "%</td><td>使用率超出  " + jvm + "%,,发送邮箱提示 </td></tr>"
				+ "</tbody>" + "</table>";
		mark = mark.replaceAll("'", "\"");
		if (!StringUtils.isEmpty(cpubool) || !StringUtils.isEmpty(jvmbool)
				|| !StringUtils.isEmpty(rambool)) {
			try {
				...
				if (StringUtils.isNotEmpty(fromEmail)
						&& StringUtils.isNotEmpty(emailName)
						&& StringUtils.isNotEmpty(emailPassword)) {
					EmailUtils.sendMail(fromEmail, email, emailName,
