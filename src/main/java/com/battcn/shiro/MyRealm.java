...
public class MyRealm extends AuthorizingRealm {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(
		...
		if (loginName != null) {
			...
			List<ResourcesEntity> reList = resourcesService
					.findResourcessByUserId(userId);
			for (ResourcesEntity resource : reList) {
				String key = resource.getResUrl();
				if (StringUtils.isNotBlank(key)) {
					key = key.substring(key.indexOf("/") + 1,
							key.lastIndexOf(".")).replace("/", ":");
		 ...
		 * SimpleAuthorizationInfo(); Set<String> setList = new
		 * HashSet<String>(); Set<String> setPermList = new HashSet<String>();
		 * List<RoleEntity> list =
		 * roleService.findRoleByUser(user.getId().toString()); for (RoleEntity
		 * roleEntity : list) { setList.add(roleEntity.getId().toString()); }
		 * authorizationInfo.setRoles(setList); Map<String,String> queryMap =
		 * new HashMap<String,String>(); queryMap.put("userId",
		 * user.getId().toString()); List<ResourcesEntity> reList =
		 * resourcesService.findRes(queryMap); for (ResourcesEntity resource :
		 * reList) { String key = resource.getResUrl(); //key =
		 * key.substring(key.indexOf("/"),key.lastIndexOf("/"));
		 * setPermList.add(key); } return null;
		 */
