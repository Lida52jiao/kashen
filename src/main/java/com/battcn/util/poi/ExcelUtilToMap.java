...
public class ExcelUtilToMap {
	 ...
	 * @throws Exception
	 */
	public static List<Map<String, String>> readExcelFileMap(String sRealPath,
			...
			for (; j < iRowNum; j++, iCellNum = 0) {
				...
				for (int k = 0; k < iCellNum; k++) {
					...
					case Cell.CELL_TYPE_STRING: // 文本类型
						aValues.put(key[k],
								String.valueOf((cellTmp.getStringCellValue())));
						break;
					case Cell.CELL_TYPE_NUMERIC: { // 文本类型
	 ...
	 * @throws Exception
	 */
	public static List<Map<String, String>> readExcelFileMap(InputStream is,
			int iSheetNum, String key[], boolean isTitle) throws Exception {
	 ...
	 * @author
	 */
	public static void createExcelFile(Workbook workBook, String sFileName,
		...
		String fileSuffix = sFileName.substring(sFileName.lastIndexOf("."),
				sFileName.length());
		// 判断文件名是否正确的扩展名(.xls)
		if (sFileName != null && !sFileName.equals("")) {
			if (sFileName.lastIndexOf(".xlsx") == -1
					|| sFileName.lastIndexOf(".xls") == -1) {
		...
		// 创建一个工作簿
		if (workBook == null) {
			workBook = fileSuffix.equals(".xls") ? new HSSFWorkbook()
					: new XSSFWorkbook();
		}
		// 获取EXCEL的样式实例
		...
		if (aList != null && aList.size() > 0) {
			...
			for (int i = 0; i < len; i++, iCountNum++) {
				...
				// 获取数据
				Map<String, String> aValues = (Map<String, String>) aList
						.get(i);
				...
				if (sTypes == null || sTypes.length == 0) {
					...
					for (String m : aValues.values()) {
						Cell cell = row.createCell((short) j); // 创建数据单元
						cell.setCellStyle(cellStyle);
						if (sTypes[j].equals(ExcelUtil.STYLE_STRING)) {// 文本类型
							cell.setCellType(Cell.CELL_TYPE_STRING);
							cell.setCellValue(m);
						} else if (sTypes[j].equals(ExcelUtil.STYLE_NUMERIC)) {// 数字类型
							cell.setCellType(Cell.CELL_TYPE_NUMERIC);
							cell.setCellValue(Double.parseDouble(m));
						} else if (sTypes[j].equals(ExcelUtil.STYLE_DATE1)) { // 日期型(必须在HSSFStyle设置)
							/**
		...
		if (response != null) {
			...
			String sSimpleName = sFileName.substring(
					sFileName.lastIndexOf("/") + 1, sFileName.length());
			response.setHeader("Content-Disposition", "attachment;filename="
					+ new String(sSimpleName.getBytes(), "ISO-8859-1"));
			long s1 = System.currentTimeMillis();
			ServletOutputStream out_s = response.getOutputStream();
			BufferedOutputStream bosTmp = new BufferedOutputStream(out_s,
					1024 * 1024 * 20);
			// 把相应的Excel 工作簿存盘
