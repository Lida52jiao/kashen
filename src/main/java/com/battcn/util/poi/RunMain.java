...
public class RunMain {
	...
	private static void readExcelToObj() throws Exception,
			FileNotFoundException {
		System.out
				.println("-----------------------------华丽分割线--路径解析：默认解析--------------------------------");
		List<KeyEntity> list1 = (List<KeyEntity>) excelUtil
				.adapterReadObj(EXCEL_PATH);
		for (KeyEntity entity : list1) {
			System.out.println(entity.getKey1() + "\t" + entity.getKey2());
		}
		System.out
				.println("-----------------------------华丽分割线--路径解析：不解析头部--------------------------------");
		List<KeyEntity> list2 = (List<KeyEntity>) excelUtil.adapterReadObj(
				EXCEL_PATH, false);
		for (KeyEntity entity : list2) {
			System.out.println(entity.getKey1() + "\t" + entity.getKey2());
		}
		System.out
				.println("-----------------------------华丽分割线--流解析：默认解析--------------------------------");
		List<KeyEntity> list3 = (List<KeyEntity>) excelUtil
				.adapterReadObj(new FileInputStream(EXCEL_PATH));
		for (KeyEntity entity : list3) {
			System.out.println(entity.getKey1() + "\t" + entity.getKey2());
		}
		System.out
				.println("-----------------------------华丽分割线--流解析：不解析头部--------------------------------");
		List<KeyEntity> list4 = (List<KeyEntity>) excelUtil.adapterReadObj(
				new FileInputStream(EXCEL_PATH), false);
		for (KeyEntity entity : list4) {
			System.out.println(entity.getKey1() + "\t" + entity.getKey2());
		}
	}

	private static void readExcelToMap() throws Exception,
			FileNotFoundException {
		System.out
				.println("-----------------------------华丽分割线--路径解析：默认解析--------------------------------");
		List<Map<String, String>> list1 = ExcelUtil.adapterReadMap(EXCEL_PATH,
				key);
		...
		System.out
				.println("-----------------------------华丽分割线--路径解析：不解析头部--------------------------------");
		List<Map<String, String>> list2 = ExcelUtil.adapterReadMap(EXCEL_PATH,
				key, false);
		...
		List<Map<String, String>> list3 = ExcelUtil.adapterReadMap(
		...
		System.out
				.println("-----------------------------华丽分割线：流解析--不解析头部--------------------------------");
		List<Map<String, String>> list4 = ExcelUtil.adapterReadMap(
				new FileInputStream(file), key, false);
		for (Map<String, String> map : list4) {
			System.out.println(map.get("key1") + "\t" + map.get("key2"));
		}
	}

	private static void readExceToArray() throws Exception,
			FileNotFoundException {
		System.out
				.println("-----------------------------华丽分割线--路径解析：默认解析--------------------------------");
		...
		System.out
				.println("-----------------------------华丽分割线：流解析--默认解析--------------------------------");
		File file = new File(EXCEL_PATH);
		List<String[]> list3 = ExcelUtil.adapterReadArray(new FileInputStream(
				file));
		...
		System.out
				.println("-----------------------------华丽分割线：流解析--不解析头部--------------------------------");
		List<String[]> list4 = ExcelUtil.adapterReadArray(new FileInputStream(
				file), false);
