...
public class TreeUtil {
	 ...
	 * @return String
	 */
	public static List<ResourcesEntity> getChildResourceForms(
			List<ResourcesEntity> list, Long praentId) {
		List<ResourcesEntity> returnList = new ArrayList<ResourcesEntity>();

		for (Iterator<ResourcesEntity> iterator = list.iterator(); iterator
				.hasNext();) {
			ResourcesEntity t = (ResourcesEntity) iterator.next();
			// 一、根据传入的某个父节点ID,遍历该父节点的所有子节点
			if (t.getParentId().longValue() == praentId.longValue()) {
				recursionFn(list, t);
				returnList.add(t);
			}
		}
	 ...
	 */
	private static void recursionFn(List<ResourcesEntity> list,
			ResourcesEntity t) {
	...
	// 得到子节点列表
	private static List<ResourcesEntity> getChildList(
			List<ResourcesEntity> list, ResourcesEntity t) {
	 ...
	 *            子节点前缀
	 */
	public List<ResourcesEntity> getChildResourceForms(
			List<ResourcesEntity> list, int typeId, String prefix) {
		if (list == null)
			return null;
		for (Iterator<ResourcesEntity> iterator = list.iterator(); iterator
				.hasNext();) {
	...
	private void recursionFn(List<ResourcesEntity> list, ResourcesEntity node,
			String p) {
	...
	// 判断是否有子节点
	private static boolean hasChild(List<ResourcesEntity> list,
			ResourcesEntity t) {
