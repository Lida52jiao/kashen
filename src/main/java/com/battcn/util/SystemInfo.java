...
public class SystemInfo {
	...
	public static JSONObject memory(Sigar sigar) {
		...
		try {
			Runtime r = Runtime.getRuntime();
			monitorMap.put("jvmTotal", div(r.totalMemory(), (1024 * 1024), 2)
					+ "M");// java总内存
			monitorMap.put("jvmUse",
					div(r.totalMemory() - r.freeMemory(), (1024 * 1024), 2)
							+ "M");// JVM使用内存
			monitorMap.put("jvmFree", div(r.freeMemory(), (1024 * 1024), 2)
					+ "M");// JVM剩余内存
			monitorMap.put("jvmUsage",
					div(r.totalMemory() - r.freeMemory(), r.totalMemory(), 2));// JVM使用率

			Mem mem = sigar.getMem();
			// 内存总量
			monitorMap.put("ramTotal",
					div(mem.getTotal(), (1024 * 1024 * 1024), 2) + "G");// 内存总量
			monitorMap.put("ramUse",
					div(mem.getUsed(), (1024 * 1024 * 1024), 2) + "G");// 当前内存使用量
			monitorMap.put("ramFree",
					div(mem.getFree(), (1024 * 1024 * 1024), 2) + "G");// 当前内存剩余量
			monitorMap.put("ramUsage", div(mem.getUsed(), mem.getTotal(), 2));// 内存使用率

			Swap swap = sigar.getSwap();
			// 交换区总量
			monitorMap.put("swapTotal",
					div(swap.getTotal(), (1024 * 1024 * 1024), 2) + "G");
			// 当前交换区使用量
			monitorMap.put("swapUse",
					div(swap.getUsed(), (1024 * 1024 * 1024), 2) + "G");
			// 当前交换区剩余量
			monitorMap.put("swapFree",
					div(swap.getFree(), (1024 * 1024 * 1024), 2) + "G");
			monitorMap
					.put("swapUsage", div(swap.getUsed(), swap.getTotal(), 2));//
	...
	public static JSONObject usage(Sigar sigar) {
		...
		try {
			...
			monitorMap.put(
					"jvmUsage",
					Math.round(div(r.totalMemory() - r.freeMemory(),
							r.totalMemory(), 2) * 100));// JVM使用率

			Mem mem = sigar.getMem();
			// 内存总量
			monitorMap.put("ramUsage",
					Math.round(div(mem.getUsed(), mem.getTotal(), 2) * 100));// 内存使用率
	...
	public static List<JSONObject> cpuInfos(Sigar sigar) {
		...
		try {
			...
			for (CpuPerc cpuPerc : cpuList) {
				JSONObject monitorMap = new JSONObject();
				monitorMap.put("cpuUserUse",
						Math.round(cpuPerc.getUser() * 100));// 用户使用率
				...
				monitorMap.put("cpuTotal",
						Math.round(cpuPerc.getCombined() * 100));// 总的使用率
