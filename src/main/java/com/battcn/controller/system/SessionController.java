...
public class SessionController {
	...
	public String list() {
		...
		for (Session session : sessions) {
			session.setAttribute("entity", (UserEntity) session
					.getAttribute(UserEntityUtil.USER_SESSION_KEY));
		}
	...
	public List<JSONObject> querySessionForList() {
		...
		if (sessions != null && sessions.size() > 0) {
			for (Session session : sessionDAO.getActiveSessions()) {
				...
				result.put("lastAccessTime", DateFormatUtils.format(
						session.getLastAccessTime(), "yyyy-MM-dd HH:mm:ss"));
				result.put("startTimestamp", DateFormatUtils.format(
						session.getStartTimestamp(), "yyyy-MM-dd HH:mm:ss"));
		...
		try {
			...
			if (session != null) {
				session.setAttribute(
						UserEntityUtil.USER_SESSION_LOGOUT_LOGOUT_KEY,
						Boolean.TRUE);
			}
		} catch (Exception e) {
			e.printStackTrace();
