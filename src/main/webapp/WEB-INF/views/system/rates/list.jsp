			...
			<form role="form" class="form-inline" id="floorNumber">
				...
				<div class="input-group">
					<button type="button" class="btn btn btn-primary"
						onclick="javascript:RateSearchForm();">
						<i class="fa fa-search"></i> 搜索
					</button>
				</div> 
			</form>
			<div class="table-responsive">
				<table id="RateTable" data-toolbar="#toolbar"
...
function queryCode() {
	return $.map($("#RateTable").bootstrapTable('getSelections'), function(
			row) {
		return row.merId
	});
}
 function queryRate() {
	...
	battcn.ajaxOpen({
		title : '调整',
		href : rootPath + '/Rates/RateList.shtml?agentid=' + box,
		width : '40%',
		height : '80%',
		okhandler : function() {
			save();
...
<script type="text/javascript">
	function getRateIdSelectionsmerCode() {
		return $.map($("#RateTable").bootstrapTable('getSelections'), function(
				row) {
			return row.merId
		});
	}
	function getRateIdSelectionsmerCodeT() {
		return $.map($("#RateTable").bootstrapTable('getSelections'), function(
				row) {
	 ...
	 function editRate() {
		var cbox = getRateIdSelectionsmerCode();
		var merChantId = getRateIdSelectionsmerCodeT();
		if (cbox == "") {
			layer.msg("请选择编辑项！！");
			return;
		}
		...
		battcn.ajaxOpen({
			title : '调整',
			href : rootPath + '/Rates/editRate.shtml?merId=' + cbox+'&merChantId='+merChantId,
			width : '40%',
			height : '50%',
			okhandler : function() {
				save();
	...
	$('#RateTable').bootstrapTable({
		...
		columns : [ {
			...
			field : 'repayMentUpdateName',
			title : '修改人姓名',
			align : 'center',
			valign : 'middle',
		},  {
			field : 'repayMentUpdateDate',
			title : '修改时间',
			align : 'center',
			valign : 'middle',
			sortable: true,
